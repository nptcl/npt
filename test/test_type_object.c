...
{
	if (parse_type_values_(Execute_Thread, ret, pos, Nil)) {
 ...
 *  main
 */
static int testcase_type_object(void)
{
...
static void testinit_type_object(Execute ptr)
{
	build_lisproot(ptr);
	build_constant();
	build_object();
...
{
	DegradeTitle;
	return DegradeCode(type_object);
}
