...
static int test_CopyJmpBuf(void)
{
	...
	test(buffer[0] == 0xAA, "CopyJmpBuf1");
	test(buffer[1] == 0xAA, "CopyJmpBuf2");
	test(buffer[sizeof(jmp_buf) - 1] == 0xAA, "CopyJmpBuf3");
	test(buffer[sizeof(jmp_buf)] == 0x00, "CopyJmpBuf4");
...
static int test_WtType(void)
{
	...
	test(buffer[7] == 0xAA, "WtType1");
	test(buffer[8 + sizeof(value)] == 0xAA, "WtType2");
	test(buffer[8] != 0xAA, "WtType3");
	value = 0;
	RdType(buffer + 8, value);
	test(value == 333, "RdType");
...
static int test_WtTypePtr(void)
{
	...
	test(buffer[15] == 0xAA, "WtTypePtr1");
	test(buffer[16 + sizeof(value)] == 0xAA, "WtTypePtr2");
	test(buffer[16] != 0xAA, "WtTypePtr3");
	value = 0;
	RdTypePtr(buffer + 16, ptr);
	test(value == 444, "RdTypePtr");
...
static int test_WtByte(void)
{
	...
	test(buffer[3] == 3, "WtByte1");
	test(buffer[9] == 9, "WtByte2");
	test(buffer[10] == 0xAA, "WtByte3");
	test(RdByte(buffer + 3) == 3, "ReadByte1");
	test(RdByte(buffer + 9) == 9, "ReadByte2");
	test(RdByte(buffer + 10) == 0xAA, "ReadByte3");
...
static int test_Align8Cut(void)
{
	...
	test(PointerMask == (Align8Mask - 1UL + 8UL), "Align8Mask");
...
static int test_Align8Front(void)
{
	...
	Align8Front(pos, &cut);
	test(pos == cut, "Align8Front1");

	a = (addr)(((pbyte)pos) - 1);
	Align8Front(a, &cut);
	test(pos == cut, "Align8Front2");

	a = (addr)(((pbyte)pos) + 3);
	Align8Front(a, &cut);
	test(pos < cut, "Align8Front3");
...
static int test_Align8Inplace(void)
{
	...
	test(pos == cut, "Align8Inplace1");

	a = (addr)(((pbyte)pos) - 1);
	cut = (addr)Align8Inplace(a);
	test(pos == cut, "Align8Inplace2");

	a = (addr)(((pbyte)pos) + 3);
	cut = (addr)Align8Inplace(a);
	test(pos < cut, "Align8Inplace3");
...
static int test_AlignSize8Cut(void)
{
	...
	test(IndexMask == (Index8Mask - 1UL + 8UL), "AlignSize8Mask");
...
static int test_AlignSize8Front(void)
{
	...
	test(pos == cut, "AlignSize8Front1");

	a = pos - 1;
	AlignSize8Front(a, &cut);
	test(pos == cut, "AlignSize8Front2");

	a = pos + 3;
	AlignSize8Front(a, &cut);
	test(pos < cut, "AlignSize8Front3");
...
static int test_AlignSize8Inplace(void)
{
	...
	test(pos == cut, "AlignSize8Inplace1");

	a = pos - 1;
	cut = AlignSize8Inplace(a);
	test(pos == cut, "AlignSize8Inplace2");

	a = pos + 3;
	cut = AlignSize8Inplace(a);
	test(pos < cut, "AlignSize8Inplace3");
...
static int test_sizeoft(void)
{
	...
	test(sizeoft(value1) == (size_t)sizeof(int), "sizeoft1");
	test(sizeoft(value2) == (size_t)sizeof(char[123]), "sizeoft2");
	test(sizeoft(value3) == (size_t)sizeof(struct value3type), "sizeoft3");
...
static int test_sizeofm(void)
{
	...
	test(sizeofm(value1, 3) == (size_t)(3*sizeof(int)), "sizeofm1");
	test(sizeofm(value2, 4) == (size_t)(4*sizeof(char[123])), "sizeofm2");
	test(sizeofm(value3, 5) == (size_t)(5*sizeof(struct value3type)), "sizeofm3");
...
static int test_clearmemory(void)
{
	...
	test(value == 0, "clearmemory1");
	memset(strp, 0xAA, sizeof(struct checkstr));
	clearmemory(strp, sizeof(struct checkstr));
	test(strv.a == 0, "clearmemory2");
	test(strv.b == 0, "clearmemory3");
	test(strv.c == 0, "clearmemory4");

	/* clearpoint */
	memset(valuep, 0xAA, sizeof(int));
	clearpoint(valuep);
	test(value == 0, "clearpoint1");
	memset(strp, 0xAA, sizeof(struct checkstr));
	clearpoint(strp);
	test(strv.a == 0, "clearpoint2");
	test(strv.b == 0, "clearpoint3");
	test(strv.c == 0, "clearpoint4");
	...
	memset(strp, 0xAA, sizeof(struct checkstr));
	cleartype(strv);
	test(strv.a == 0, "cleartype2");
	test(strv.b == 0, "cleartype3");
	test(strv.c == 0, "cleartype4");

	/* cleartype */
	memset(valuea, 0xAA, sizeof(int) * 10);
	clearsize(valuea, 3);
	test(valuea[0] == 0, "clearsize1");
	test(valuea[1] == 0, "clearsize2");
	test(valuea[2] == 0, "clearsize3");
	test(valuea[3] != 0, "clearsize4");
	memset(stra, 0xAA, sizeof(struct checkstr) * 10);
	clearsize(stra, 3);
	test(stra[0].a == 0, "clearsize5");
	test(stra[1].b == 0, "clearsize6");
	test(stra[2].c == 0, "clearsize7");
	test(stra[3].a != 0, "clearsize8");
...
static int test_aamemory(void)
{
	...
	cleartype(mem);
	aamemory(mem, 10);
	test(mem[0] == 0xAA, "aamemory1");
	test(mem[9] == 0xAA, "aamemory2");
	test(mem[10] == 0, "aamemory3");

	cleartype(mem);
	aatype(mem);
	test(mem[0] == 0xAA, "aatype1");
	test(mem[99] == 0xAA, "aatype2");
...
static int test_GetBitByte(void)
{
	...
	v = 4;
	test(GetBitByte(v, 1) == 0, "GetBitByte1");
	test(GetBitByte(v, 2) != 0, "GetBitByte2");
	test(GetBitByte(v, 3) == 0, "GetBitByte3");
	SetBitByte(v, 3, 1);
	test(GetBitByte(v, 1) == 0, "GetBitByte4");
	test(GetBitByte(v, 2) != 0, "GetBitByte5");
	test(GetBitByte(v, 3) != 0, "GetBitByte6");
	test(GetBitByte(v, 4) == 0, "GetBitByte7");
	SetBitByte(v, 2, 0);
	test(GetBitByte(v, 1) == 0, "GetBitByte8");
	test(GetBitByte(v, 2) == 0, "GetBitByte9");
	test(GetBitByte(v, 3) != 0, "GetBitByte10");
	test(GetBitByte(v, 4) == 0, "GetBitByte11");
