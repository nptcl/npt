...
static int test_close_result(void)
{
	...
	test(check != Unbound, "close_result1");
	test(consp(cons), "close_result2");
	GetControl(pos, Control_Result, &cons);
	test(cons == Nil, "close_result3");
	...
	close_result(pos);
	GetCons(list, &check, &cons);
	test(check == Unbound, "close_result4");
	GetCons(cons, &check, &cons);
	test(check == Unbound, "close_result5");
	test(cons == Nil, "close_result6");
	GetControl(pos, Control_Result, &cons);
	test(cons == Nil, "close_result7");
...
static int test_close_type(void)
{
	addr prev, next, pos, value1, value2, cons, check;
	struct control_struct *str;
	Execute ptr;
	...
	GetResultControl(next, 0, &check);
	test(check == Unbound, "close_type1");
...
static int test_close_lexical(void)
{
	...
	test(RefFixnum(check) == 20, "close_lexical1");
	getlexical_local(ptr, sym2, &check);
	test(RefFixnum(check) == 40, "close_lexical2");

	close_lexical(ptr, next);
	getlexical_local(ptr, sym1, &check);
	test(RefFixnum(check) == 111, "close_lexical3");
	getlexical_local(ptr, sym2, &check);
	test(RefFixnum(check) == 222, "close_lexical4");
...
static int test_close_special(void)
{
	addr pos, next, sym1, sym2, check;
	Execute ptr;
	...
	test(RefFixnum(check) == 20, "close_special1");
	getspecial_local(ptr, sym2, &check);
	test(RefFixnum(check) == 40, "close_special2");

	close_special(ptr, next);
	getspecial_local(ptr, sym1, &check);
	test(RefFixnum(check) == 111, "close_special3");
	getspecial_local(ptr, sym2, &check);
	test(RefFixnum(check) == 222, "close_special4");
...
static int test_close_function(void)
{
	addr pos, next, sym1, sym2, check;
	Execute ptr;
	...
	test(RefFixnum(check) == 20, "close_function1");
	getfunction_local(ptr, sym2, &check);
	test(RefFixnum(check) == 40, "close_function2");

	close_function(ptr, next);
	getfunction_local(ptr, sym1, &check);
	test(RefFixnum(check) == 111, "close_function3");
	getfunction_local(ptr, sym2, &check);
	test(RefFixnum(check) == 222, "close_function4");
...
static int test_close_setf(void)
{
	addr pos, next, sym1, sym2, check;
	Execute ptr;
	...
	test(RefFixnum(check) == 20, "close_setf1");
	getsetf_local(ptr, sym2, &check);
	test(RefFixnum(check) == 40, "close_setf2");

	close_setf(ptr, next);
	getsetf_local(ptr, sym1, &check);
	test(RefFixnum(check) == 111, "close_setf3");
	getsetf_local(ptr, sym2, &check);
	test(RefFixnum(check) == 222, "close_setf4");
 ...
 */
static int test_call_callbind_system(void)
{
	test(CallBindTable[CallBind_system] == call_callbind_system,
			"call_callbind_system1");
	RETURN;
}

static int test_function_value;
static void test_function_any(Execute ptr)
{
...
static int test_call_callbind_any(void)
{
	...
	setcompiled_any(pos, test_function_any);
...
static int test_call_callbind_empty(void)
{
	...
	setcompiled_empty(pos, test_function_any);
	test_function_value = 0;
	call_compiled_function(ptr, pos);
...
static void test_function_dynamic(Execute ptr, addr cons)
{
...
static int test_call_callbind_dynamic(void)
{
	...
	setcompiled_dynamic(pos, test_function_dynamic);
	test_function_value = -1;
	call_compiled_function(ptr, pos);
...
static void test_function_rest(Execute ptr, addr cons)
{
...
static int test_call_callbind_rest(void)
{
	...
	setcompiled_rest(pos, test_function_rest);
	test_function_value = -1;
	call_compiled_function(ptr, pos);
...
static void test_function_var1(Execute ptr, addr var1)
{
...
static int test_call_callbind_var1(void)
{
	...
	setcompiled_var1(pos, test_function_var1);
...
static void test_function_var2(Execute ptr, addr var1, addr var2)
{
...
static int test_call_callbind_var2(void)
{
	...
	setcompiled_var2(pos, test_function_var2);
	setargs_va_control(ptr, fixnumh(10), fixnumh(20), NULL);
	call_compiled_function(ptr, pos);
...
static void test_function_var3(Execute ptr, addr var1, addr var2, addr var3)
{
	test_function_value =
...
static int test_call_callbind_var3(void)
{
	...
	setcompiled_var3(pos, test_function_var3);
...
static void test_function_opt1(Execute ptr, addr var1)
{
...
static int test_call_callbind_opt1(void)
{
	...
	setcompiled_opt1(pos, test_function_opt1);
	setargs_nil_control(ptr);
	setvalues_nil_control(ptr);
	call_compiled_function(ptr, pos);
...
static void test_function_opt2(Execute ptr, addr var1, addr var2)
{
	if (var1 == Unbound)
		test_function_value = -1;
...
static int test_call_callbind_opt2(void)
{
	...
	setcompiled_opt2(pos, test_function_opt2);
	setargs_nil_control(ptr);
	setvalues_nil_control(ptr);
	call_compiled_function(ptr, pos);
...
static void test_function_opt3(Execute ptr, addr var1, addr var2, addr var3)
{
	if (var1 == Unbound) {
...
static int test_call_callbind_opt3(void)
{
	...
	setcompiled_opt3(pos, test_function_opt3);
	setargs_nil_control(ptr);
	setvalues_nil_control(ptr);
	call_compiled_function(ptr, pos);
...
static void test_function_var1opt1(Execute ptr, addr var1, addr var2)
{
	if (var2 == Unbound) {
		test_function_mode = 1;
		test_function_value = (int)RefFixnum(var1);
...
static int test_call_callbind_var1opt1(void)
{
	...
	setcompiled_var1opt1(pos, test_function_var1opt1);
	setargs_va_control(ptr, fixnumh(10), NULL);
	setvalues_nil_control(ptr);
	call_compiled_function(ptr, pos);
...
static void test_function_var2opt1(Execute ptr, addr var1, addr var2, addr var3)
{
	if (var3 == Unbound) {
		test_function_mode = 2;
		test_function_value = (int)(RefFixnum(var1) + RefFixnum(var2));
...
static int test_call_callbind_var2opt1(void)
{
	...
	setcompiled_var2opt1(pos, test_function_var2opt1);
	setargs_va_control(ptr, fixnumh(10), fixnumh(20), NULL);
	setvalues_nil_control(ptr);
	call_compiled_function(ptr, pos);
...
static void test_function_var1opt2(Execute ptr, addr var1, addr var2, addr var3)
{
	if (var2 == Unbound) {
		test_function_mode = 1;
		test_function_value = (int)RefFixnum(var1);
...
static int test_call_callbind_var1opt2(void)
{
	...
	setcompiled_var1opt2(pos, test_function_var1opt2);
	setargs_va_control(ptr, fixnumh(10), NULL);
	setvalues_nil_control(ptr);
	call_compiled_function(ptr, pos);
...
static void test_function_var1rest(Execute ptr, addr var1, addr rest)
{
	addr pos;
	fixnum value;
...
static int test_call_callbind_var1rest(void)
{
	...
	setcompiled_var1rest(pos, test_function_var1rest);
	setargs_va_control(ptr, fixnumh(10), NULL);
	setvalues_nil_control(ptr);
	call_compiled_function(ptr, pos);
...
static void test_function_var2rest(Execute ptr, addr var1, addr var2, addr rest)
{
	addr pos;
	fixnum value;
...
static int test_call_callbind_var2rest(void)
{
	...
	setcompiled_var2rest(pos, test_function_var2rest);
	setargs_va_control(ptr, fixnumh(10), fixnumh(20), NULL);
	setvalues_nil_control(ptr);
	call_compiled_function(ptr, pos);
 ...
 *  execute
 */
static void test_function_length_code(Execute ptr, addr right)
{
...
static void test_make_code(addr *ret)
{
	...
	setcompiled_system(call, test_function_length_code);
	symbol_heap(&pos);
...
static void test_function_length(Execute ptr, addr cons)
{
...
static int test_execute_control(void)
{
	...
	/* compiled */
	compiled_heap(&call, Nil);
	setcompiled_dynamic(call, test_function_length);
	setargs_va_control(ptr, fixnum_heapr(10), fixnum_heapr(20), NULL);
...
static void test_execute_go_set(Execute ptr, addr right)
{
...
static int test_execute_go(void)
{
	...
	setcompiled_system(pos, test_execute_go_set);
...
static int testbreak_control(void)
{
	...
	TestBreak(test_close_type);
	TestBreak(test_close_lexical);
	TestBreak(test_close_special);
	TestBreak(test_close_function);
	TestBreak(test_close_setf);
	...
	/* call_compiled_function */
	TestBreak(test_call_callbind_system);
