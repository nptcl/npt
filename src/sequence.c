...
static int listp_sequence(addr pos)
{
	switch (GetType(pos)) {
		case LISPTYPE_NIL:
		case LISPTYPE_CONS:
		...
		addr a, size_t s1, addr b, size_t s2)
{
	...
	/* merge */
loop:
	if (call_sort_sequence(str, &check, a2, b2)) return 1;
	if (check) {
		setcar(a, a1);
		getcdr(a, &a);
		ai++;
		...
		addr a, addr c, size_t s0)
{
	...
	/* simple-sort */
	if (s0 < MERGE_SORT_LIMIT)
		return simple_sort_list_sequence(str, a, c);
		...
		addr pos, size_t a, size_t c)
{
	...
	s0 = c - a;
	if (s0 < MERGE_SORT_LIMIT)
		return simple_sort_vector_sequence(str, pos, a, c);
