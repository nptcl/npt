...
static void defmacro_lambda(void)
{
	...
	setcompiled_macro(pos, function_lambda);
...
static void defun_eval(void)
{
	...
	setcompiled_var1(pos, function_eval);
...
static void defmacro_defmacro(void)
{
	...
	compiled_macro_heap(&pos, symbol);
	setcompiled_macro(pos, function_defmacro);
	SetMacroCommon(symbol, pos);
	/* type */
	GetCallType(&type, Compiled_MacroFunction);
	settype_function(pos, type);
...
static void defun_macro_function(void)
{
	...
	compiled_heap(&pos, symbol);
	setcompiled_var1opt1(pos, function_macro_function);
	SetFunctionCommon(symbol, pos);
	/* type */
...
static void defun_setf_macro_function(void)
{
	...
	GetConst(COMMON_MACRO_FUNCTION, &symbol);
	compiled_heap(&pos, symbol);
	setcompiled_var2opt1(pos, function_setf_macro_function);
...
static void defun_macroexpand(void)
{
	...
	compiled_heap(&pos, symbol);
	setcompiled_var1opt1(pos, function_macroexpand);
	SetFunctionCommon(symbol, pos);
	/* type */
	GetCallType(&type, Compiled_MacroExpand);
	settype_function(pos, type);
	settype_function_symbol(symbol, type);
}
...
static void defun_macroexpand_1(void)
{
	...
	compiled_heap(&pos, symbol);
	setcompiled_var1opt1(pos, function_macroexpand_1);
	SetFunctionCommon(symbol, pos);
	/* type */
	GetCallType(&type, Compiled_MacroExpand);
	settype_function(pos, type);
...
static void defmacro_define_symbol_macro(void)
{
	...
	compiled_macro_heap(&pos, symbol);
	setcompiled_macro(pos, function_define_symbol_macro);
	SetMacroCommon(symbol, pos);
	/* type */
	GetCallType(&type, Compiled_MacroFunction);
	settype_function(pos, type);
...
static void function_declaim(Execute ptr, addr form, addr env)
{
	...
	parse_declaim_alloc(NULL, form, &form);
...
static void defmacro_declaim(void)
{
	...
	compiled_macro_heap(&pos, symbol);
	setcompiled_macro(pos, function_declaim);
	SetMacroCommon(symbol, pos);
	/* type */
	GetCallType(&type, Compiled_MacroFunction);
	settype_function(pos, type);
...
static void defun_special_operator_p(void)
{
	...
	compiled_heap(&pos, symbol);
	setcompiled_var1(pos, function_special_operator_p);
	SetFunctionCommon(symbol, pos);
	/* type */
...
static void defun_constantp(void)
{
	...
	compiled_heap(&pos, symbol);
	setcompiled_var1opt1(pos, function_constantp);
	SetFunctionCommon(symbol, pos);
	/* type */
	type_constantp(&type);
	settype_function(pos, type);
